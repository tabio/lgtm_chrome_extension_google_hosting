  ...
  atwhoOptions = {
    ...
    callbacks: {
      ...
      remote_filter: function(query, callback) {
        ...
        switch (false) {
          case kind !== 'a':
            return $.getJSON(chrome.extension.getURL('/config/animal.json'), function(target) {
              get_images(target, callback)
            });
            break;
          case kind !== 'b':
            return $.getJSON(chrome.extension.getURL('/config/bijyo.json'), function(target) {
              get_images(target, callback)
            });
            break;
          case kind !== 'd':
            return $.getJSON(chrome.extension.getURL('/config/drawing.json'), function(target) {
              get_images(target, callback)
            });
            break;
          case kind !== 'g':
            return $.getJSON(chrome.extension.getURL('/config/gohan.json'), function(target) {
              get_images(target, callback)
            });
            break;
          case kind !== 'j':
            return $.getJSON(chrome.extension.getURL('/config/japan.json'), function(target) {
              get_images(target, callback)
            });
            break;
          case kind !== 'v':
            return $.getJSON(chrome.extension.getURL('/config/vehicle.json'), function(target) {
              get_images(target, callback)
            });
            break;
          case kind !== 'w':
            return $.getJSON(chrome.extension.getURL('/config/wafuku.json'), function(target) {
              get_images(target, callback)
            });
            break;
        }
